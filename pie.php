...
$r = Database::getInstance()->select("select sum(amount), description from Credit where HOUR(timestamp) >= HOUR(NOW()) and DATE(timestamp) = CURDATE() group by description order by sum desc;");
